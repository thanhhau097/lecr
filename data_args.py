...
class DataArguments:
    ...
    top_k_neighbors: int = field(
        default=50,
        metadata={"help": "select top_k nearest neighbors for training and valiation set"},
    )
    ...
    translated_correlation_path: str = field(
        default="./data/translated_correlations.csv",
        metadata={"help": "translated correlation csv path"},
    )
    use_sampler: bool = field(default=False, metadata={"help": "Use custom sampler"})
    pn_ratio: int = field(default=1, metadata={"help": "Positive/negative ratio"})
    use_augmentation: bool = field(default=False, metadata={"help": "Use augmentation for text"})
    use_no_content_topics: bool = field(
        default=False, metadata={"help": "Use augmentation for text"}
    )
    topic_embs_dict_path: str = field(
        default="data/mpnet-topic-embs-fold0.pth", metadata={"help": "topic embeddings dict path"}
    )
    content_embs_dict_path: str = field(
        default="data/mpnet-content-embs-fold0.pth",
        metadata={"help": "content embeddings dict path"},
    )
    knn_path: str = field(default="data/knn_preds_fold0.csv", metadata={"help": "knn preds path"})
