...
class ProportionalTwoClassesBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        minority = np.split(
            y_indices[0][
                : (len(y_indices[0]) // self.minority_size_in_batch) * self.minority_size_in_batch
            ],
            len(y_indices[0]) // self.minority_size_in_batch,
        )
        ...
        majority = np.split(
            y_indices[1][: (len(y_indices[1]) // majority_size_in_batch) * majority_size_in_batch],
            len(y_indices[1]) // majority_size_in_batch,
        )
        ...
        indices = np.concatenate(
            [np.random.permutation(np.concatenate([a, b])) for a, b in zip(minority, majority)]
        ).tolist()
